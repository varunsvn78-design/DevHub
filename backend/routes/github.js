...
router.get('/dev/:username', async (req, res, next) => {
  try {
    ...
    return res.json({ profile, repos, stats: { totalStars, totalForks, topLanguages } });
  } catch (err) {
    return next(err);
  }
});
...
router.get('/repo/:owner/:name', async (req, res, next) => {
  try {
    ...
    const [languages, contributors, activity, issues] = await Promise.all([
      gh.getLanguages(owner, name).catch(() => ({})),
      gh.getContributors(owner, name, 10).catch(() => []),
      gh.getCommitActivity(owner, name).catch(() => []),
      gh.getOpenIssues(owner, name, 5).catch(() => []),
    ]);
    ...
    return res.json({ repo, languages: langs, languageDistribution, contributors: contribList, activity: activityList, openIssues });
  } catch (err) {
    return next(err);
  }
});
